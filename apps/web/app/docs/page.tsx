 ...
 * 레이아웃 레퍼런스: EarthIsMine/Vigil api-docs (사이드바 + 앵커 섹션),
 * 배경·색은 나루 세계관 유지. 내용의 원본은 CLAUDE.md / 개발 명세서 v1.3.
 */
...
const SECTIONS: DocSection[] = [
  ...
  { id: "chain-config", title: "체인 파라미터 주입" },
  { id: "contracts", title: "컨트랙트" },
  { id: "metrics", title: "지표 정의" },
  { id: "identity", title: "신원 검증" },
  { id: "analysis", title: "온체인 분석" },
  { id: "seed", title: "데모 시드 데이터" },
        ...
        <div className="mt-10 gap-10 lg:grid lg:grid-cols-[210px_minmax(0,1fr)]">
          ...
          <div className="min-w-0 rounded-xl bg-black/[0.18] px-7 py-2">
            <Section id="overview" title="개요">
              <p>
                나루는 기와체인(GIWA)의 <Term>가격 데이터 레이어</Term>와, 그
                위에 올린 <Term>업비트 유저용 온체인 터미널 · 검증
                런치패드</Term>입니다. 두 층으로 나뉩니다.
              </p>
...
[상단] 터미널 + 검증 런치패드  하단 API의 첫 번째 소비자이자 쇼케이스`}
            ...
            <Section id="architecture" title="아키텍처">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={k} className="border-b border-black/25 last:border-0">
                      <td className="py-2 pr-4 font-mono text-[12px] text-ink-3">{k}</td>
              ...
              <p>
                ...
                . 체결 이벤트는 라우터가 아니라 <Term>페어가 발행</Term>하므로
                라우터를 우회한 직접 체결도 전량 잡힙니다. 리오그는 Ponder의
                체크포인트·롤백이 처리하고, 같은 블록을 두 번 처리해도 결과가
                같습니다.
              </p>
            </Section>
            ...
            <Section id="contracts" title="컨트랙트">
              <p>
                Uniswap V2 포크(Factory / Pair / Router)에 발행 게이트와 신원
                원장을 더했습니다. 전 컨트랙트가 Blockscout에 소스
                검증(verify)되어 있습니다.
              </p>
              <table className="w-full text-[12.5px]">
                <tbody>
              ...
              <p>
                발행은 <Term>issueAndList 단일 경로</Term>입니다: 신원
                원장(attest)을 통과한 주체만 호출할 수 있고, 공급량이 고정되고
                추가발행·전송 과세·오너 권한이 없는 토큰을 만들어 페어 생성과
                초기 유동성 공급까지 한 트랜잭션으로 끝냅니다. 검증 근거의
                지문(identityRef)은 발행 시점에 온체인에 영구 기록됩니다.
              </p>
            </Section>

            <Section id="metrics" title="지표 정의">
              <p>
                모든 화폐 계산은 bigint로 하고 표시 직전에만 문자열로
                변환합니다. 정의가 코드보다 우선합니다.
              </p>
              <table className="w-full text-[12.5px]">
                <tbody>
                  {[
                    ...
                    <tr key={k} className="border-b border-black/25 last:border-0">
                      <td className="w-24 py-2 pr-4 align-top font-medium text-ink">{k}</td>
            ...
            <Section id="analysis" title="온체인 분석">
              <p>
                발행 토큰의 <Term>Transfer를 전수 인덱싱</Term>해 홀더 잔고
                원장과 지갑 간 직접 전송 그래프를 만듭니다. 발행자·팀 지갑은
                심사에서 등록되므로 라벨이 추론이 아니라 기록입니다.
              </p>
              <ul className="list-disc space-y-1 pl-5">
                <li>
                  홀더 관계도: 버블 = 상위 홀더(크기 = 보유 비중), 선 = 두 지갑
                  간 직접 전송 이력
                </li>
                <li>
                  클러스터 = 연결 성분(union-find). 그래프를 전체 전송 참여자
                  위에서 만들고 홀더로 투영해, 잔고 0인 경유 지갑을 통한 간접
                  연결도 같은 클러스터로 묶입니다
                </li>
                <li>
              ...
              <table className="w-full text-[12.5px]">
                <tbody>
                  {[
                    ...
                    <tr key={k} className="border-b border-black/25 last:border-0">
                      <td className="w-24 py-2 pr-4 font-medium text-ink">{k}</td>
                      <td className="py-2 text-ink-2">{v}</td>
                    </tr>
                  ))}
                </tbody>
