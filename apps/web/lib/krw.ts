...
// 업저씨가 실제로 기준점으로 삼는 종목만 - USDT 는 환전 수단이라 뺐다(절대 규칙 3)
 ...
 * 출처(업비트)를 화면에 명시한다. 실패 시 빈 배열 - 표시 정책(잠깐의 실패는
 * 마지막 값 유지, 연속 실패는 바 숨김)은 클라이언트(naru-feed-dock) 몫이다.
 *
 * 캐시 1초: 클라이언트가 1초 폴링해도 업비트 원천 호출은 서버에서 초당
 * 1회로 합쳐진다(업비트 시세 API 제한 대비 여유). 환산율(getEthKrw)의
 * 60초 TTL 과는 별개 - 이건 표시용 체결가이지 환산 소스가 아니다.
 */
